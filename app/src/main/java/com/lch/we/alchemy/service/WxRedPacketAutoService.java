 ...
 */
public class WxRedPacketAutoService extends AccessibilityService {

    private static final String ChatList = "com.tencent.mm.ui.LauncherUI";
    /**
     * 消息列表页面Activity类名
     */
    private static final String MessageList = "com.tencent.mm.ui.LauncherUI";
    /**
     * 红包页面Activity类名
     */
     ...
     * 红包详情页面Activity类名
     */
    private static final String RedEnvelopeDetail = "com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyDetailUI";
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        switch (event.getEventType()) {
            ...
            case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                ...
                if ("com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyNotHookReceiveUI".equals(currentActivity)){
...
//            }
    ...
    private void openPacket(AccessibilityEvent event) {
        ...
        if (nodeInfo != null) {
            ...
            if (resultList.size() > 0 || resultList2.size() > 0) { // 红包已抢完
                ...
                /* 戳开红包，红包还没抢完，遍历节点匹配“拆红包” */
                List<AccessibilityNodeInfo> buttons = new ArrayList<AccessibilityNodeInfo>();
                findOpenButtonByType(nodeInfo,"android.widget.Button",buttons);
                for (AccessibilityNodeInfo node2:buttons) {
    ...
    private boolean watchList(AccessibilityEvent event) {
        ...
        if ( eventSource == null)
            return false;
        String searchText = "[微信红包]";
        List<AccessibilityNodeInfo> nodes = new ArrayList<AccessibilityNodeInfo>();
        findOpenButtonByTypeAndKey(eventSource,nodes,"android.view.View",searchText);

//        findOpenButtonByType(eventSource,"android.widget.ListView",nodes);
//        List<AccessibilityNodeInfo> nodes1 = new ArrayList<AccessibilityNodeInfo>();
//        if (!nodes.isEmpty()) {
//            findOpenButtonByKey(nodes.get(0),nodes1,searchText);
//        }
//
        if (!nodes.isEmpty()) {
    ...
    public AccessibilityNodeInfo getLastRedpackageNode(AccessibilityNodeInfo rootNode, String search) {
        ...
        if (rootNode != null) {
            ...
            //nodeInfoList.get(0).getParent().getParent().getChild(1).getChild(1)
            //android.widget.RelativeLayout
            boolean[] res = new boolean[nodeInfoList.size()];
            for(int i= 0;i<nodeInfoList.size();i++){
                AccessibilityNodeInfo info1 = nodeInfoList.get(i);
                res[i] = false;
                if(nodeInfoList.size() < 1) return  null;
                if(info1.getParent().getChildCount() > 1) {
                    if(info1.getParent().getChild(1).getText() != null &&
                            info1.getParent().getChild(1).getText().equals("已领取")){
                        res[i] = true;
                        continue;
                    }
                }
                AccessibilityNodeInfo rel = info1.getParent().getParent();
                if(rel.getClassName().equals("android.widget.RelativeLayout")){
                    if(rel.getChildCount() > 1){
                        if(rel.getChild(1).getChildCount() > 1){
                            if(rel.getChild(1).getChild(1).getText() != null &&
                                    rel.getChild(1).getChild(1).getText().equals("已领取")){
                                res[i] = true;
                            }
                        }
                    }
...
//            LogUtil.d( "nodeInfoList=" + nodeInfoList);
            if (nodeInfoList != null && nodeInfoList.size() > 0) {
                ...
                for(int i= 0;i<nodeInfoList.size();i++){
                    if(res[i]) continue;
                    AccessibilityNodeInfo node = nodeInfoList.get(i);
                    if (node != null) {
    ...
    private void findOpenButtonByType(AccessibilityNodeInfo node,String type,List<AccessibilityNodeInfo> visited) {
        if (node == null)
            return;

        //非layout元素
        if (node.getChildCount() == 0) {
            if (type.equals(node.getClassName())){
                visited.add(node);
                return;
            }
            else
        ...
        for (int i = 0; i < node.getChildCount(); i++) {
            findOpenButtonByType(node.getChild(i),type,visited);
        }
    }
     ...
     * @param keyName
     */
    private void findOpenButtonByTypeAndKey(AccessibilityNodeInfo node,List<AccessibilityNodeInfo> visited,String type ,String keyName) {
        if (node == null)
            return;
        if(visited.size() > 1){
            //取第一个
        ...
        if (node.getChildCount() == 0) {
            if (type.equals(node.getClassName())){
                if(node.getText() != null
                        && !node.getText().toString().equals("")
                        && node.getText().toString().contains(keyName)){
                    visited.add(node);
                }
            }
        }
        ...
        for (int i = 0; i < node.getChildCount(); i++) {
            findOpenButtonByTypeAndKey(node.getChild(i),visited,type,keyName);
        }
    }
